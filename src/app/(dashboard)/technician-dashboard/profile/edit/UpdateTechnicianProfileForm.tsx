...
export default function UpdateTechnicianProfileForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      profilePhoto: (fd.get("profilePhoto") as string) || undefined,
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
            ...
            <button
              type="button"
              onClick={() => router.push("/technician-dashboard/profile")}
              className="px-5 py-3 border border-border rounded-xl text-sm font-medium text-foreground hover:bg-muted transition-colors"
            >
              Cancel
            </button>
          </div>
